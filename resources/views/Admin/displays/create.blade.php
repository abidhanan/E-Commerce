...
@section('content')
    <div class="container-fluid">
        <div class="mb-4">
            <h4 class="mb-1">Create Konfigurasi Display</h4>
            <p class="text-muted mb-0">Buat banner, teks berjalan, dan tautan kampanye yang akan tayang di beranda.</p>
        </div>
        ...
        <form action="{{ route('admin.displays.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
                        ...
                        <div class="card mb-4 border-0 shadow-sm">
                            <div class="card-header bg-white border-bottom border-light d-flex justify-content-between align-items-center">
                                ...
                                <div class="form-check form-switch m-0">
                                    <input class="form-check-input" type="checkbox" role="switch" name="{{ $activeField }}" id="{{ $activeField }}" checked value="1">
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-5 mb-3 mb-md-0">
                                        <label class="form-label text-muted small fw-bold text-uppercase">Unggah Gambar</label>
                                        ...
                                        <small class="text-muted d-block mt-2" style="font-size: 11px;">Maks 3MB (JPG/PNG/WEBP)</small>
                                    </div>

                                    <div class="col-md-7">
                                        <div class="mb-3">
                                            <label class="form-label text-muted small fw-bold text-uppercase">Judul Utama</label>
                                            <input type="text" name="{{ $title }}" class="form-control" placeholder="Teks besar (Misal: BUILD YOUR)" value="{{ old($title) }}">
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label text-muted small fw-bold text-uppercase">Sub-Judul</label>
                                            <input type="text" name="{{ $subTitle }}" class="form-control" placeholder="Teks pendukung (Misal: Exclusive Collaboration)" value="{{ old($subTitle) }}">
                                        </div>
                                        <div>
                                            <label class="form-label text-muted small fw-bold text-uppercase">Tautan / URL Tujuan <i class="bi bi-link-45deg"></i></label>
                                            <input type="url" name="{{ $link }}" class="form-control" placeholder="https://domain.com/collection/..." value="{{ old($link) }}">
                    ...
                    <div class="card mb-4 border-0 shadow-sm sticky-top" style="top: 20px;">
                        ...
                        <div class="card-body">
                            <div class="mb-4">
                                ...
                                <textarea name="running_text" class="form-control" rows="4" placeholder="Misal: FREE SHIPPING ON ALL ORDERS OVER RP 500.000">{{ old('running_text') }}</textarea>
                            </div>
